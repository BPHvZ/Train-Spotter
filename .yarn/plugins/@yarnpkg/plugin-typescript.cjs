			...
			var e = {
									...
									afterWorkspaceDependencyAddition: async (e, t, r, o) => {
										...
										const g = `${a.suggestUtils.Modifier.CARET}${y.major}`,
											...
											f = s.miscUtils.mapAndFind(i.workspaces, (e) => {
												var t, a;
												const o =
														null === (t = e.manifest.dependencies.get(r.identHash)) ||
														void 0 === t
															? void 0
															: t.descriptorHash,
													n =
														null === (a = e.manifest.devDependencies.get(r.identHash)) ||
														void 0 === a
															? void 0
															: a.descriptorHash;
					...
					712: (e, t, r) => {
						...
						function s(e = { serializable: !0 }) {
							...
							return {
								...
								set: (r, s) => (
									(t[JSON.stringify(r)] = e.serializable ? JSON.stringify(s) : s), Promise.resolve(s)
								),
					...
					223: (e, t, r) => {
						...
						const n = (e) => {
							...
							i = (e) => (t, r) =>
								e.transporter.write({ method: o.N.Post, path: "2/abtests", data: t }, r),
							c = (e) => (t, r) =>
								e.transporter.write({ method: o.N.Delete, path: (0, s.encode)("2/abtests/%s", t) }, r),
							d = (e) => (t, r) =>
								e.transporter.read({ method: o.N.Get, path: (0, s.encode)("2/abtests/%s", t) }, r),
					...
					757: (e, t, r) => {
						...
						function o(e, t = (e, t) => Promise.resolve()) {
							return Object.assign(e, {
								wait: (r) => o(e.then((e) => Promise.all([t(e, r), e])).then((e) => e[1])),
							});
						}
						function n(e) {
							let t = e.length - 1;
					...
					103: (e, t, r) => {
						...
						const n = (e) => {
							...
							i = (e) => (t) =>
								e.transporter.read({ method: o.N.Get, path: "1/strategies/personalization" }, t),
							c = (e) => (t, r) =>
								e.transporter.write(
									{ method: o.N.Post, path: "1/strategies/personalization", data: t },
									r
								);
					},
					790: (e, t, r) => {
						...
						const c = (e) => {
							const t = e.appId,
								...
								n = {
									...
									clearCache: () =>
										Promise.all([o.requestsCache.clear(), o.responsesCache.clear()]).then(() => {}),
								};
						...
						const p = (e) => (t, r) => {
							...
							m = (e) => (t, r, s) => {
								...
								return (
									...
									e.transporter.write(
										{ method: o.N.Post, path: "1/clusters/mapping", data: { cluster: r } },
										n
									)
								);
							},
							...
							O = (e) => (t, r) =>
								e.transporter.read({ method: o.N.Get, path: (0, s.encode)("1/keys/%s", t) }, r),
							...
							N = (e) => (t) =>
								e.transporter.read({ method: o.N.Get, path: "1/clusters/mapping/top" }, t),
							S = (e) => (t, r) =>
								e.transporter.read(
									{ method: o.N.Get, path: (0, s.encode)("1/clusters/mapping/%s", t) },
									r
								),
							D = (e) => (t) => {
							...
							k = (e) => (t, r) =>
								(0, s.createWaitablePromise)(
									e.transporter.write(
										{ method: o.N.Post, path: "1/indexes/*/batch", data: { requests: t } },
										r
									),
									(t, r) =>
										Promise.all(
											Object.keys(t.taskID).map((s) =>
												x(e)(s, { methods: { waitTask: De } }).waitTask(t.taskID[s], r)
											)
										)
								),
							U = (e) => (t, r) =>
								e.transporter.read(
									{ method: o.N.Post, path: "1/indexes/*/objects", data: { requests: t } },
									r
								),
							C = (e) => (t, r) => {
								const s = t.map((e) => ({
							...
							W = (e) => (t, r) =>
								(0, s.createWaitablePromise)(
									e.transporter.write(
										{ method: o.N.Post, path: (0, s.encode)("1/keys/%s/restore", t) },
										r
									),
									(r, a) =>
										(0, s.createRetryablePromise)((r) =>
							...
							F = (e) => (t, r) =>
								e.transporter.read(
									{ method: o.N.Post, path: "1/clusters/mapping/search", data: { query: t } },
									r
								),
							G = (e) => (t, r) => {
								...
								return (0, s.createWaitablePromise)(
									e.transporter.write(
										{ method: o.N.Put, path: (0, s.encode)("1/keys/%s", t), data: c },
										i
									),
									(r, o) =>
										(0, s.createRetryablePromise)((r) =>
							...
							$ = (e) => (t) =>
								(0, s.createWaitablePromise)(
									e.transporter.write(
										{ method: o.N.Post, path: (0, s.encode)("1/indexes/%s/clear", e.indexName) },
										t
									),
									(t, r) => De(e)(t.taskID, r)
								),
								...
								return (
									...
									(0, s.createWaitablePromise)(
										e.transporter.write(
											{
												method: o.N.Post,
												path: (0, s.encode)("1/indexes/%s/synonyms/clear", e.indexName),
											},
											i
										),
										(t, r) => De(e)(t.taskID, r)
							...
							_ = (e) => (t) =>
								(0, s.createWaitablePromise)(
									e.transporter.write(
										{ method: o.N.Delete, path: (0, s.encode)("1/indexes/%s", e.indexName) },
										t
									),
									(t, r) => De(e)(t.taskID, r)
								),
							...
							se = (e) => (t, r) =>
								e.transporter.read(
									{ method: o.N.Get, path: (0, s.encode)("1/indexes/%s/%s", e.indexName, t) },
									r
								),
							ae = () => (e, t) => {
								for (const [r, s] of Object.entries(e.hits))
							...
							oe = (e) => (t, r) => {
								...
								return e.transporter.read(
									{ method: o.N.Post, path: "1/indexes/*/objects", data: { requests: n } },
									a
								);
							},
							ne = (e) => (t, r) =>
							...
							de = (e) => (t, r) =>
								e.transporter.read(
									{
										method: o.N.Get,
										path: (0, s.encode)("1/indexes/%s/task/%s", e.indexName, t.toString()),
									},
									r
								),
							le = (e) => (t, r) =>
								...
								return (
									...
									(0, s.createWaitablePromise)(
										e.transporter.write(
											{
												method: o.N.Post,
												path: (0, s.encode)("1/indexes/%s/synonyms/batch", e.indexName),
												data: t,
											},
											d
										),
